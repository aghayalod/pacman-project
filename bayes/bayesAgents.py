...
def fillObsCPT(bayesNet, gameState):
    ...
    #4 houses * 7 walls = 28 observations
    for house in gameState.getPossibleHouses():
        for walls in gameState.getHouseWalls(house):
            observation = OBS_VAR_TEMPLATE % walls
            factor = bn.Factor([observation], [GHOST_HOUSE_VAR, FOOD_HOUSE_VAR], bayesNet.variableDomainsDict())

            #assign position of adjacent house based on if wall and house position of each of the 4 next to each other
            position = None
            if (abs(walls[0] - topLeftPos[0]) <= 1) and (abs(walls[1] - topLeftPos[1]) <= 1):
                position = TOP_LEFT_VAL
            if (abs(walls[0] - bottomLeftPos[0]) <= 1) and (abs(walls[1] - bottomLeftPos[1]) <= 1):
                position = BOTTOM_LEFT_VAL
            if (abs(walls[0] - bottomRightPos[0]) <= 1) and (abs(walls[1] - bottomRightPos[1]) <= 1):
                position = BOTTOM_RIGHT_VAL
            if (abs(walls[0] - topRightPos[0]) <= 1) and (abs(walls[1] - topRightPos[1]) <= 1):
                position = TOP_RIGHT_VAL
            
            #assign probability based on assignment for position
            ...
            for assignment in factor.getAllPossibleAssignmentDicts():
                ...
                if position == assignment[FOOD_HOUSE_VAR]:
                    if assignment[observation] == RED_OBS_VAL:
                        probability = PROB_FOOD_RED
                    elif assignment[observation] == BLUE_OBS_VAL:
                ...
                elif position == assignment[GHOST_HOUSE_VAR]:
                    if assignment[observation] == RED_OBS_VAL:
                ...
                elif assignment[observation] == NO_OBS_VAL:                   
                        probability = 1
                factor.setProbability(assignment, probability)
            
            bayesNet.setCPT(observation, factor)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    assms = factor.getAllPossibleAssignmentDicts()
    for assm in assms:
        if fin_prob < factor.getProbability(assm):
            fin_prob = max(factor.getProbability(assm), fin_prob)
            dictionary = assm
    return dictionary
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        "*** YOUR CODE HERE ***"
        vars = [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR]
        sample = inference.inferenceByVariableElimination(self.bayesNet, vars, evidence, eliminationOrder)
        ...
        leftProb = sample.getProbability(left)
        rightProb = sample.getProbability(right)

        leftExpectedValue = rightProb * GHOST_COLLISION_REWARD + leftProb * WON_GAME_REWARD
        rightExpectedValue = rightProb * WON_GAME_REWARD + leftProb * GHOST_COLLISION_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for prob, val in probsAndOutcomes:
            var = max(self.computeEnterValues(val, enterEliminationOrder))
